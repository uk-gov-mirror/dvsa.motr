...
import uk.gov.dvsa.motr.notifier.events.HgvPsvTwoMonthEmailReminderEvent;
...
public class HgvPsvTwoMonthEmailNotification extends SendableEmailNotification {
    ...
    private final String mothDirectUrlPrefix;
    private final String checksumSalt;

    public HgvPsvTwoMonthEmailNotification(String webBaseUrl, String mothDirectUrlPrefix, String checksumSalt) {
        super(webBaseUrl);
        this.mothDirectUrlPrefix = mothDirectUrlPrefix;
        this.checksumSalt = checksumSalt;
        setNotificationPathSubject(HGV_PSV_DIRECTORY + "hgv-psv-two-month-notification-email-subject.txt");
        setNotificationPathBody(HGV_PSV_DIRECTORY + "hgv-psv-two-month-notification-email-body.txt");
    }
    ...
    public void personalise(SubscriptionQueueItem subscription, VehicleDetails vehicleDetails) {
        ...
        if (vehicleHadItsFirstMotTest(vehicleDetails)) {
            String preservationStatement = HGV_PSV_PRESERVATION_STATEMENT_PREFIX +
    ...
    public NotifyEvent getEvent() {
        return new HgvPsvTwoMonthEmailReminderEvent();
    }
}
