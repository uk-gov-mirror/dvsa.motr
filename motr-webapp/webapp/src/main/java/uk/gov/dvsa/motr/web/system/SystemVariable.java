package uk.gov.dvsa.motr.web.system;

import uk.gov.dvsa.motr.config.ConfigKey;

public enum SystemVariable implements ConfigKey {

    LOG_LEVEL("LOG_LEVEL"),
    REGION("REGION"),
    GOV_NOTIFY_URI("GOV_NOTIFY_URI"),
    // must be secret
    GOV_NOTIFY_API_TOKEN("GOV_NOTIFY_API_TOKEN"),
    MOT_TEST_REMINDER_INFO_API_URI("MOT_TEST_REMINDER_INFO_API_URI"),
    MOT_TEST_REMINDER_BY_VRM_API_URI("MOT_TEST_REMINDER_BY_VRM_API_URI"),
    SUBSCRIPTION_CONFIRMATION_TEMPLATE_ID("CONFIRMATION_TEMPLATE_ID"),
    EMAIL_CONFIRMATION_TEMPLATE_ID("CONFIRM_EMAIL_NOTIFICATION_TEMPLATE_ID"),
    SMS_SUBSCRIPTION_CONFIRMATION_TEMPLATE_ID("SMS_CONFIRMATION_TEMPLATE_ID"),
    SMS_CONFIRM_PHONE_TEMPLATE_ID("SMS_CONFIRM_PHONE_TEMPLATE_ID"),
    // must be secret
    MOT_TEST_REMINDER_INFO_TOKEN("MOT_TEST_REMINDER_INFO_TOKEN"),
    STATIC_ASSETS_HASH("STATIC_ASSETS_HASH"),
    STATIC_ASSETS_URL("STATIC_ASSETS_URL"),
    DB_TABLE_PENDING_SUBSCRIPTION("DB_TABLE_PENDING_SUBSCRIPTION"),
    DB_TABLE_SMS_CONFIRMATION("DB_TABLE_SMS_CONFIRMATION"),
    DB_TABLE_SUBSCRIPTION("DB_TABLE_SUBSCRIPTION"),
    DB_TABLE_CANCELLED_SUBSCRIPTION("DB_TABLE_CANCELLED_SUBSCRIPTION"),
    BASE_URL("BASE_URL"),
    RELEASE_VERSION("RELEASE_VERSION"),
    DO_WARM_UP("WARM_UP"),
    MOT_TEST_REMINDER_INFO_API_CLIENT_READ_TIMEOUT("MOT_TEST_REMINDER_INFO_API_CLIENT_READ_TIMEOUT"),
    MOT_TEST_REMINDER_INFO_API_CLIENT_CONNECTION_TIMEOUT("MOT_TEST_REMINDER_INFO_API_CLIENT_CONNECTION_TIMEOUT"),
    WARM_UP_TIMEOUT_SEC("WARM_UP_TIMEOUT_SEC"),

    // Feature toggles
    FEATURE_TOGGLE_HGV_PSV_VEHICLES("FEATURE_TOGGLE_HGV_PSV_VEHICLES"),
    FEATURE_TOGGLE_TRAILERS("FEATURE_TOGGLE_TRAILERS");

    final String name;

    SystemVariable(String name) {
        this.name = name;
    }

    @Override
    public String getName() {
        return name;
    }
}
